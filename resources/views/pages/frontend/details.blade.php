              ...
              <div class="px-3 flex-none" style="width: 320px">
                <div class="rounded-xl p-4 pb-8 relative bg-white">
                  <div class="rounded-xl overflow-hidden card-shadow w-full h-36">
                    <img
                      src="{{ $recoms->galleries()->exists() ? Storage::url($recoms->galleries->first()->url) : 'data:image/gif;base64,R0lGODlhAQABAIAAAMLCwgAAACH5BAAAAAAALAAAAAABAAEAAAICRAEAOw=='}}"
                      alt=""
                      class="w-full h-full object-cover object-center"
                    />
                  </div>
                  <h5 class="text-lg font-semibold mt-4">{{$recoms->name}}</h5>
                  <span class="">IDR {{$recoms->price}}</span>
                  <a href="{{route('details', $recoms->slug)}}" class="stretched-link">
                    <!-- fake children -->
                  </a>
                </div>
              </div>
