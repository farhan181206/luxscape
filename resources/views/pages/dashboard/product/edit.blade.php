                ...
                <form action="{{ route('dashboard.product.update' , $product->id)}} " class="w-full" method="post" enctype="multipart/form-data">
                    ...
                    <div class="flex-warp -mx-3 mb-3">
                        <div class="w-full px-3">
                            ...
                            <input type="text" value="{{old('name') ?? $product->name}}" name="name" class="block w-full bg-grey-200 text-grey-700 border border-grey-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 mb-2" placeholder="Product Name">
                        </div>
                        <div class="w-full px-3">
                            <label for="" class="block uppercase tracking-wide text-grey-700 text-xs font-bold mb-2">Description</label>
                            <textarea name="description" class="block w-full bg-grey-200 text-grey-700 border border-grey-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" >{{old('desciption') ?? $product->description}}</textarea>
                        </div>
                        <div class="w-full px-3 mt-2">
                            <label for="" class="block uppercase tracking-wide text-grey-700 text-xs font-bold mb-2">Price</label>
                            <input type="number" value="{{old('price') ?? $product->price}}" name="price" class="block w-full bg-grey-200 text-grey-700 border border-grey-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 mb-2" placeholder="Product Price">
                        </div>
                        <div class="w-full px-3 mt-2">
